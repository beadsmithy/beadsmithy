...
Email validation logic was duplicated across 4 modules. Extracted into `utils/validators.ts` so
...
The dashboard was importing all chart types upfront, adding ~180KB to the initial load. Charts are
now loaded on demand, cutting initial load time by ~40% on slow connections.
```
...
New contributors were struggling to get the dev environment running. Added step-by-step
instructions covering Node version, env vars, and the database seed command.
```
...
The production image included dev dependencies and build cache artifacts,
making deploys slower and increasing registry storage. The Dockerfile now
uses a multi-stage build so only runtime files are copied into the final
image.
```
