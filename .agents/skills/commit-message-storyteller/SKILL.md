...
description: 'Analyzes git diffs or staged changes and generates narrative commit messages that explain WHY a change was made, not just what changed — following Conventional Commits format. Use when asked to "write a commit message", "generate a commit", "describe my changes", "commit this", "summarize my diff", or "commit your changes". Works with git diff output, staged files, or plain descriptions of changes.'
---
...
| `build`    | Build system, packaging, bundling, or dependency/build-tool changes |
...
- [ ] Scope identifies the affected area, or is omitted when global
- [ ] Subject is imperative, lowercase after the colon, and has no period
- [ ] Subject stays under 72 characters
- [ ] Body explains the reason, context, or impact instead of restating the diff
- [ ] Breaking changes use `!` and/or a `BREAKING CHANGE:` footer
- [ ] Issue references are included when available
- [ ] Unrelated changes are split into separate commit messages
