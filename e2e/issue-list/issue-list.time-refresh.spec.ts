 ...
 * epoch milliseconds. Set once the deferred Issue exists so the final
 * test waits for the real boundary rather than re-deriving one. */
...
const expectNoRefreshFailureBanner = async (): Promise<void> => {
  ...
  await browser.waitUntil(async () => !(await banner.isExisting()), {
    timeout: 30_000,
    timeoutMsg: "Expected no refresh-failure banner during time refresh",
  });
};
...
describe("Issue explorer (WebDriver e2e): time-trigger refresh moves a deferred Issue into Ready without a ref move", () => {
  ...
  it("publishes the post-launch deferred Issue as not Ready through the ref poll", async () => {
    ...
    // Ready transition attributable to the time trigger alone.
  ...
  it("moves the deferred Issue into Ready through the time trigger after the boundary passes", async () => {
    ...
    // The Issue's status stays `deferred` (only `bw undefer` changes
    // it); the time-derived change is Ready membership alone. The
    // Deferred view therefore still lists it.
    await expectSidebarCount("Deferred", "1 issue");
