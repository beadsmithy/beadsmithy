...
3. **Window focus gain:** when the window gains focus (not when it loses focus), we will refresh once, so returning to the app is always fresh. Every observed focus gain refreshes; we deliberately do not suppress the first focus event of a session. The refresh callback attaches during app setup, so it may miss a startup focus event — harmless, because startup already loads/restores the snapshot — and suppression could discard the user's first genuine return on platforms that emit no startup focus event. The worst case is one silent, single-flight, revision-admitted startup refresh.
4. **Single-flight loader:** all three triggers route through one single-flight loader that runs the existing Beadwork loaders (`bw list --all --json`, `bw ready --json`, `bw blocked --json`). If a reload is already in flight, a new trigger sets a dirty flag and the loader re-runs once after the active load completes if the data is still stale. The loader emits a Tauri event carrying the new `IssueExplorerData`, the observed SHA, the workspace-selection generation that owned the load, the workspace path, and a monotonic refresh revision (independent of selection generation; SHA is provenance, not an ordering token).
...
6. **Workspace switch:** on workspace switch (per ADR-0006), we will trigger an immediate load for the new workspace and start the poller for subsequent ticks. We will keep a per-workspace `last_seen_sha` map so a re-visited workspace whose Beadwork ref has not moved does not run a _second_ full Issue Explorer load after the switch's own validate/load/commit transaction lands; the switch transaction itself still runs its normal three-view loader. A fresh workspace (no prior SHA) always fires the switch's load and additionally seeds the SHA map under the same runtime lock from a before/after pair of `git rev-parse` probes. An SHA mismatch between the two probes — or a probe failure on either side — leaves the entry unseeded so the coordinator reconciles on the next tick. (`bsm-wj1.2` scope decision: skipping the switch transaction's normal loader on an unchanged revisit would require a per-Workspace Issue snapshot cache plus invalidation for time-sensitive Ready membership; that is out of scope.)
7. **No manual refresh button:** we will not ship one in the UI.
8. **Workspace generation guard:** every emitted refresh event carries the workspace path, the workspace-selection generation that owned the load, and a monotonic refresh revision. The selection generation is incremented by the existing `WorkspaceService` on every successful switch (and survives across refreshes for the same identity); the refresh revision is the coordinator's independent monotonic counter. A workspace switch rebinds the renderer's confirmed identity and resets the refresh revision so a delayed loader from a previous workspace cannot overwrite the new workspace's state. The renderer admits only events for the currently-rendered (path, selection generation) pair with a strictly newer refresh revision.
